const {
    SlashCommandBuilder, EmbedBuilder, ButtonBuilder, PermissionFlagsBits, ActionRowBuilder,
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) => subcommand.setName("view").setDescription("View all your set up teams."))
        .addSubcommand((subcommand) => subcommand
            .setName("ready_check")
            .setDescription("Send a ready check to all the teams in your list."))
        .addSubcommand((subcommand) => subcommand
            .setName("remove")
            .setDescription("Remove a team from your list.")
            .addIntegerOption((option) => option
                .setName("team_index")
                .setDescription("The team number that you wish to remove. Find with /manage_team view.")
                .setRequired(true)))
        .addSubcommand((subcommand) => subcommand
            .setName("info")
            .setDescription("Sends out an initial message that alerts players of their team channel(s)."))
        .addSubcommand((subcommand) => subcommand
            .setName("purge")
...
async function addChannels(interaction) {
    ...
    if (!teamChannels) {
        ...
        await teamChannelSchema.create({
            guildId: interaction.guild.id, teams: values,
        });
    } else {
    ...
    const newTeamEmbedField = {
        name: `Team ${teamChannels?.teams.length ?? 1}`, value: `Team Role:\n<@&${values[1]}>
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`Successfully added team channels for team ${teamChannels?.teams.length ?? 1}.`)
...
async function viewChannels(interaction) {
    ...
    const embed = new EmbedBuilder()
        .setTitle("Team Channels")
        .setDescription(`Use the buttons below to navigate through the list of team channels.`)
        .setColor(guildIconColour)
        .addFields(teamList);
...
async function readyCheck(interaction) {
    ...
    const readyButton = new ButtonBuilder({
        emoji: {
            id: "1089592997206835351", name: "yes",
        },
    })
        .setCustomId("ready")
        .setLabel("Ready")
    ...
    const notReadyButton = new ButtonBuilder({
        emoji: {
            id: "1089593104547446935", name: "no",
        },
    })
    ...
    teamChannels.teams.forEach((team) => {
        ...
        channel.send({
            content: `<@&${team[1]}>`, embeds: [embed], components: [row],
        });
    });

    await interaction.reply({
        content: `Ready check sent to all teams.`, ephemeral: true,
    });
        ...
        if (buttonInteraction.isButton()) {
            ...
            if (duplicateChecker.includes(respondantTeam[0])) {
                return await buttonInteraction.reply({
                    content: `You have already responded to the ready check.`, ephemeral: true,
                });
            }
...
async function removeChannels(interaction) {
    ...
    if (teamIndex < 1 || teamIndex > teamChannels.teams.length) {
        return await interaction.reply({
            content: `Invalid team index.`, ephemeral: true,
        });
    }
    ...
    await interaction.reply({
        content: `Successfully removed team ${teamIndex}.`, ephemeral: true,
    });
}
...
async function sendInfo(interaction) {
    ...
    teamChannelsWithVcUrl.forEach((team) => {
        const message = team[2] ? `${interaction.guild.roles.cache.get(team[1])}, please check you can see this message and the voice channel: ${team[2]}.` : `${interaction.guild.roles.cache.get(team[1])}, please check you can see this message.`;
...
async function purgeChannels(interaction) {
    ...
    const embed = new EmbedBuilder()
        .setTitle("Team Channels")
        .setDescription(`Purging all team channels. (This may take a while). If messages are older than 14 days, they will not be deleted.`)
        .setColor(guildIconColour);
