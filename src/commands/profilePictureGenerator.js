...
module.exports = {
    cooldown: 2, data: new SlashCommandBuilder()
        .setName("retrieve_icon")
        .setDescription("Gets a downloadable version of one or four people's icons.")
        ...
        .addSubcommandGroup((group) => group
            ...
            .addSubcommand((subcommand) => subcommand
                ...
                .addStringOption((option) => option
                    .setName("player_one")
                    .setDescription("The first user of whose Minecraft face you want.")
                    .setRequired(true))
                .addStringOption((option) => option
                    .setName("player_two")
                    .setDescription("The second user of whose Minecraft face you want.")
                    .setRequired(true))
                .addStringOption((option) => option
                    .setName("player_three")
                    .setDescription("The third user of whose Minecraft face you want.")
                    .setRequired(true))
                .addStringOption((option) => option
                    .setName("player_four")
                    .setDescription("The fourth user of whose Minecraft face you want.")
                    .setRequired(true))))
        .addSubcommandGroup((group) => group
            .setName("discord")
            ...
            .addSubcommand((subcommand) => subcommand
                ...
                .addStringOption((option) => option
                    ...
                    .addChoices({name: "PNG", value: "png"}, {name: "WEBP", value: "webp"}, {
                        name: "JPEG",
                        value: "jpeg"
                    })))
            .addSubcommand((subcommand) => subcommand
                ...
                .addUserOption((option) => option
                    .setName("user_one")
                    .setDescription("The first user of whose profile picture you want.")
                    .setRequired(true))
                .addUserOption((option) => option
                    .setName("user_two")
                    .setDescription("The second user of whose profile picture you want.")
                    .setRequired(true))
                .addUserOption((option) => option
                    .setName("user_three")
                    .setDescription("The third user of whose profile picture you want.")
                    .setRequired(true))
                .addUserOption((option) => option
                    .setName("user_four")
                    .setDescription("The fourth user of whose profile picture you want.")
                    .setRequired(true))
                .addStringOption((option) => option
    ...
    async execute(interaction) {
        ...
        if (subcommandGroup === "discord") {
            if (subcommand === "single") {
                ...
                const formatOfPhoto = interaction.options.getString("image_format_solo") || "png";
                ...
                const formatOfPhoto = interaction.options.getString("image_format_quad") || "png";
            ...
            if (subcommand === "single") {
                ...
                const playerArray = [interaction.options.getString("player_one"), interaction.options.getString("player_two"), interaction.options.getString("player_three"), interaction.options.getString("player_four"),];
...
async function processAvatar(interaction, user, formatOfPhoto, regex) {
    ...
    const avatarUrl = user.avatarURL({
        format: formatOfPhoto, dynamic: true, size: 1024,
    });
    ...
    try {
        ...
        const attachment = new Discord.AttachmentBuilder(await canvas.encode(formatOfPhoto), {name: `avatarOf${user.username.replace(regex, "")}.${formatOfPhoto}`});

        await interaction.reply({files: [attachment]});
    } catch (error) {
        console.error("Error while processing the avatar image:", error);
        await interaction.reply("Failed to retrieve the user's avatar.");
    }
}
...
async function processAvatarGroup(interaction, userArray, formatOfPhoto, regex) {
    ...
    for (const user of userArray) {
        if (!user.avatar) {
            ...
            const avatarUrl = user.avatarURL({
                format: formatOfPhoto, dynamic: true, size: 4096,
            });
    ...
    await interaction.reply({content: edgeCaseMessage, files: attachmentList});
}
...
async function processMinecraftAvatar(interaction, player, regex) {
    ...
    try {
        ...
        const attachment = new Discord.AttachmentBuilder(await canvas.encode("png"), {name: `avatarOf${player.replace(regex, "")}.png`});
...
async function processMinecraftAvatarGroup(interaction, playerArray, regex) {
    ...
    await interaction.reply({files: attachmentList});
}
