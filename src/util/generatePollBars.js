...
async function generatePollBars(pollMessage, votingOptions) {
    ...
    if (totalReactions > 0) {
        skyBattlePercentage = Math.round(((skyBattleVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
        battleBoxPercentage = Math.round(((battleBoxVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
        holeInWallPercentage = Math.round(((holeInWallVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
        toGetToOtherSidePercentage = Math.round(((toGetToOtherSideVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
        parkourWarriorPercentage = Math.round(((parkourWarriorVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
        dynaballPercentage = Math.round(((dynaballVotes * 100) / totalReactions + Number.EPSILON) * 100) / 100;
    }
    // Update the poll message with the new vote counts and percentages
    let pollMessageString = " ";
    if (votingOptions[0]) {
        pollMessageString += `**Battle Box**  ${options[0].emoji} ${getBar(battleBoxPercentage)} [ ${battleBoxPercentage}% • ${battleBoxVotes} ]\n\n`;
    }
    if (votingOptions[1]) {
        pollMessageString += `**Dynaball** ${options[1].emoji} ${getBar(dynaballPercentage)} [ ${dynaballPercentage}% • ${dynaballVotes} ]\n\n`;
    }
    if (votingOptions[2]) {
        pollMessageString += `**Hole In Wall**  ${options[2].emoji} ${getBar(holeInWallPercentage)} [ ${holeInWallPercentage}% • ${holeInWallVotes} ]\n\n`;
    }
    if (votingOptions[3]) {
        pollMessageString += `**Parkour Warrior: Survivor**  ${options[3].emoji} ${getBar(parkourWarriorPercentage)} [ ${parkourWarriorPercentage}% • ${parkourWarriorVotes} ]\n\n`;
    }
    if (votingOptions[4]) {
        pollMessageString += `**Sky Battle**  ${options[4].emoji} ${getBar(skyBattlePercentage)} [ ${skyBattlePercentage}% • ${skyBattleVotes} ]\n\n`;
    }
    if (votingOptions[5]) {
        pollMessageString += `**To Get To The Other Side**  ${options[5].emoji} ${getBar(toGetToOtherSidePercentage)} [ ${toGetToOtherSidePercentage}% • ${toGetToOtherSideVotes} ]\n\n`;
    }
