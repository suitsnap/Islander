const getTextColorFromBackground = require("./colours").getTextColorFromBackground;
...
function createCanvasWheel(canvas, ctx, options, endAngleDegrees, totalSteps) {
    ...
    function drawOption(angle, index, option, backgroundColor) {
        ...
        function drawPortion() {
            ...
            ctx.lineWidth = 1;
            ctx.moveTo(cx, cy);
            ctx.arc(cx, cy, radius, optionAngle, optionAngle + portionSizes[index] * Math.PI * 2, false);
            ctx.lineTo(cx, cy);
            ctx.fill();
            ctx.stroke();
        }
    ...
    return {
        rotate: rotate, getOptionByStep: getOptionByStep,
    };
}
