...
/**
 * Handles the ticket button interaction to create a ticket
 * @param {ButtonInteraction} buttonInteraction
 */
function ticketButton(buttonInteraction) {
    buttonInteraction.deferReply({ephemeral: true});
    const category = buttonInteraction.client.channels.cache.get("1054887073884151898");
    const channelName = "ticket-" + buttonInteraction.user.username.toLowerCase();
    buttonInteraction.guild.channels
        .create({
            name: channelName, type: ChannelType.GuildText, parent: category, permissionOverwrites: [{
                id: buttonInteraction.guild.id, deny: PermissionFlagsBits.ViewChannel,
            }, {
                id: "1105256862670127196", allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages,],
            }, {
        ...
        .then((textChannel) => {
            ...
            buttonInteraction.followUp("Ticket created! Please go to: " + `<#${channelID}>`);
            setTimeout(() => {
