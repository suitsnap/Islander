...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("suggestion")
        .setDescription(
          "Suggest any idea you want, and I will do my best to implement it."
        )
        .setRequired(true)
    )
    .addBooleanOption((option) =>
      option
        .setName("anonymous")
        .setDescription("If you wish to suggest anonymously")
    ),
  async execute(interaction) {
    ...
    const suggestEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: `Created by ${user}` });

    const suitsnapUser = await interaction.client.users.fetch(
      "369238700489441280"
    );

    await suitsnapUser.send({
      embeds: [suggestEmbed],
    });
